<?php
    ...
    $comm = mysql_query("select id from meizu where  username='$userName' and telephone='$phoneNumber'", $conn);
    if($comm){
        $row = mysql_num_rows($comm);
        if($row==0){
            //d用来做排序
            $com =  mysql_query("select * from meizu where id", $conn);
            $id = mysql_num_rows($com);
            // 添加
            mysql_query("insert into meizu values ($id+1,'$userName','$phoneNumber','$passWord','')", $conn);
            echo "1";
        }
    }
    ...
    // $com =  mysql_query("select pwd from meizu where userName='$userName' or telephone='$phoneNumber'", $conn);
